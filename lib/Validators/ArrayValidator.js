...
module.exports = skmatc.create(function(schema) {
    ...
    if(!Array.isArray(data)) return this.fail('Expected ' + path + ' to be an array but got ' + JSON.stringify(data) + ' instead');

    if(data.length < min) return this.fail('Expected ' + path + ' to have a length of at least ' + min + ' but got ' + data.length + ' instead'); 
    if(max >= 0 && data.length > max) return this.fail('Expected ' + path + ' to have a length of less than ' + min + ' but got ' + data.length + ' instead');

    var result = Result.compound(_.map(data, function(value, index) {
